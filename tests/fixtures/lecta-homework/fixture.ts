...
export const test = base.extend<PageFixtures> ({
    resultPage: async ({page}, use) => {
        await use(new ResultPage(page, 100));
    },

    tasksTeacherPage: async ({page}, use) => {
        await use(new TasksTeacherPage(page, 100));
    },
    ...
    mainPageDirector: async ({page}, use) => {
        await use(new MainPageDirector(page, 'ГБОУ ЛИЦЕЙ № 126 КАЛИНИНСКОГО РАЙОНА САНКТ-ПЕТЕРБУРГА', 'Купить больше лицензий'));
    },
