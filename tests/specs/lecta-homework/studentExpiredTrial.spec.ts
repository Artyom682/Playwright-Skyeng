...
test('Студент без лицензий', async ({ lectaMainPage, getHomeworkPage, selectSubjectPage, selectTaskPage, authPage,  tasksStudentPage, page }) => {
  ...
  for (let i = 0; i < lectaSubjects.subjects.length; i++) {
    await selectSubjectPage.subjectCardWithLockIconButton(subjects[i]).waitUntilElementIsVisible();
  }
...
test('Наличие двух кнопок', async ({ lectaMainPage, tasksTeacherPage, getHomeworkPage, selectSubjectPage, selectTaskPage, authPage,  tasksStudentPage, page }) => {
  await lectaMainPage.goto();
  await lectaMainPage.authRegButton.click();
  ...
  await authPage.loginProsvId('ucheniktest@mail.ru', '11111111');
  await getHomeworkPage.buyingLicenseButton.isVisible();
  await getHomeworkPage.schoolAccessActivationButton.isVisible();
});
